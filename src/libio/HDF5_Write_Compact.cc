...
namespace mcpdft {
   void HDF5WriteCompact::write_rdms(const arma::mat &D1a, 
                               const arma::mat &D1b,
			       const arma::mat &D2ab) {
      write_opdm(D1a, D1b);
      write_tpdm(D2ab);
   ...
   void HDF5WriteCompact::write_opdm(const arma::mat &D1a,
		               const arma::mat &D1b) {
      ...
      /* Set the dcpls' layout to COMPACT */
      status = H5Pset_layout (dcpl_D1a, H5D_COMPACT);
      status = H5Pset_layout (dcpl_D1b, H5D_COMPACT);
   ...
   void HDF5WriteCompact::write_tpdm(const arma::mat &D2ab) {
      ...
      /* Set the dcpl layout to COMPACT */
      status = H5Pset_layout (dcpl_D2ab, H5D_COMPACT);
