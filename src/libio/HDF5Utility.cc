...
namespace mcpdft {
   ...
   void HDF5Utility::read_enuc(double &enuc) {
      /* file indentifiers and handles */
      hid_t file_id;
      hid_t enuc_dst_id;
      herr_t status;
      ...
      /* Open the existing HDF5 dataset in the read-only mode */
      enuc_dst_id  = H5Dopen2(file_id, H5D_E_NUC, H5P_DEFAULT);

      /* Read the DataSet */
      status = H5Dread(enuc_dst_id, H5T_NATIVE_DOUBLE,
                       H5S_ALL, H5S_ALL, H5P_DEFAULT, &enuc);

      /* Close datasets */
      status = H5Dclose(enuc_dst_id);
