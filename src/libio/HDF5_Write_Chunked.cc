...
namespace mcpdft {
   void HDF5WriteChunked::write_rdms(const arma::mat &D1a, 
                               const arma::mat &D1b,
			       const arma::mat &D2ab) {
      write_opdm(D1a, D1b);
      write_tpdm(D2ab);
   ...
   void HDF5WriteChunked::write_opdm(const arma::mat &D1a,
		               const arma::mat &D1b) {
      assert( CHUNK_DIM0 == CHUNK_DIM1 );
      ...
      hsize_t  chunk[2]  = {CHUNK_DIM0, CHUNK_DIM1};
      hsize_t  offset[2] = {0,0};
      hsize_t  stride[2] = {1,1};
      size_t   n_chunk   = dim/CHUNK_DIM0;
      hsize_t  count[2]  = {n_chunk, n_chunk};
      hsize_t  block[2]  = {CHUNK_DIM0, CHUNK_DIM1};
      ...
      dcpl_D1a = H5Pcreate(H5P_DATASET_CREATE);
      dcpl_D1b = H5Pcreate(H5P_DATASET_CREATE);

      /* Set the dcpls' layout to CHUNKED */
      status = H5Pset_chunk(dcpl_D1a, RANK, chunk);
      status = H5Pset_chunk(dcpl_D1b, RANK, chunk);

      /* Create a dataset in group "/D1a" */
      D1a_dst_id = H5Dcreate2(file_id, D1A_DATASET,
        	              H5T_IEEE_F64LE, dataspace_id,
                              H5P_DEFAULT, dcpl_D1a, H5P_DEFAULT);

      /* Create a dataset in group "/D1b" */
      D1b_dst_id = H5Dcreate2(file_id, D1B_DATASET,
        	              H5T_IEEE_F64LE, dataspace_id,
                              H5P_DEFAULT, dcpl_D1b, H5P_DEFAULT);
   ...
   void HDF5WriteChunked::write_tpdm(const arma::mat &D2ab) {
      assert( CHUNK_DIM0 == CHUNK_DIM1 );
      ...
      dcpl_D2ab = H5Pcreate (H5P_DATASET_CREATE);

      /* Set the dcpl layout to CHUNKED */
      status = H5Pset_chunk(dcpl_D2ab, RANK, chunk);

      /* Create a dataset in group "/D2ab" */
      D2ab_dst_id = H5Dcreate2(file_id, D2AB_DATASET,
        	              H5T_IEEE_F64LE, dataspace_id,
                              H5P_DEFAULT, dcpl_D2ab, H5P_DEFAULT);
